...
- [ ] **C2. Per-game "fitxa" pages**: small static page per game (`games/<slug>/about.html` or `/jocs/<slug>.html`) with 2-3 paragraphs of real text (what it teaches, course level, modes, screenshots) + big "Jugar" button. This is what can actually rank for "joc d'ortografia catalana online" — playable game HTML alone rarely ranks. Must live outside the robocopy /MIR target or in the landing repo root. *(fixes P9)*
