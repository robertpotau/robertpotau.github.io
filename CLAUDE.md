...
- **Fitxa pages** (2026-07-20): `jocs/<slug>.html` per game + hub `jocs/index.html` — the text-rich pages that can actually rank in Google (playable game HTML has no indexable text). Generated by `tools/gen_fitxes.py`; per-game copy lives in its `GAMES` list. New game → add entry there, re-run the script, add sitemap URL, and add the card's `ℹ️ Fitxa` link in index.html (i18n key `fitxa`). The `jocs/` folder is deliberately OUTSIDE `games/` so `sync-game.ps1`'s robocopy /MIR never touches it.
